...
Job.prototype = {
  ...
  runPhase: function (next) {
    ...
    if (this.phase === 'prepare') {
      provider = this.config.providers[this.repo.provider.id]
      tasks.push(provider.clone.bind(provider, this.providerConfig, this.repo.provider.config, self.pluginContext(this.repo.provider.id, env)))
    }
