...
function Job(task, provider, plugins, config) {
...
Job.prototype = {
  ...
  initDataDir: function () {
    ...
    var name = this.id + '-' + this.project.name.replace('/', '-')
  ...
  runPhase: function (next) {
    ...
    if (this.phase === 'prepare') {
      tasks.push(this.provider.fetch.bind(this.provider, this.providerConfig, this.job.provider.config, self.pluginContext(this.project.provider.id, env)))
    }
    this.plugins.forEach(function (plugin) {
      // all plugins will get the final, fully populated env object
      _.extend(env, plugin.getEnv(self.phase))
      tasks.push(function (next) {
        plugin.runPhase(self.phase, self.pluginContext(plugin.name, env), next)
      })
    })
  ...
  phaseDone: function (err, results) {
    ...
    if (['deploy', 'test'].indexOf(this.phase) !== -1 && sum(results) === 0) {
